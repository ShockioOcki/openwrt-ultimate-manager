...
return view.extend({
	...
	render(data) {
		...
		const explicitDirectRoutes = routingSet(podkopRouting.direct?.lists);
		const coveredRussiaServices = new Set(['meta', 'twitter', 'tiktok']);
					...
					E('div', { 'class': 'oum-passwall-overview', id: 'passwall-panel', hidden: '' }, [
						...
						E('details', { 'class': 'oum-passwall-diagnostics' }, [
							...
							E('div', { 'class': 'oum-passwall-tools' }, [
								...
								E('p', { 'class': 'oum-muted' }, 'Показывает, какое правило OUM применит к домену.'),
		...
		const selectedCommunities = (route) => Array.from(root.querySelectorAll('[data-community-route]:checked')).filter((item) => item.value === route && !(route === 'direct' && coveredRussiaServices.has(item.dataset.communityRoute) && !explicitDirectRoutes.has(item.dataset.communityRoute) && root.querySelector('[data-community-route="russia_inside"][value="vpn"]')?.checked)).map((item) => item.dataset.communityRoute).join('\n');
		...
		const updateRouteSummary = () => {
			const russiaSelected = root.querySelector('[data-community-route="russia_inside"][value="vpn"]')?.checked;
			for (const row of root.querySelectorAll('[data-route-row]')) {
				const id = row.dataset.routeRow;
				row.hidden = !!russiaSelected && coveredRussiaServices.has(id) && !explicitDirectRoutes.has(id);
				row.style.setProperty('display', row.hidden ? 'none' : '', row.hidden ? 'important' : '');
			}
			for (const pill of document.querySelectorAll('[data-route-pill]')) {
				const row = root.querySelector('[data-route-row="' + pill.dataset.routePill + '"]');
				pill.style.display = row?.hidden ? 'none' : 'inline-flex';
			}
		...
		const beginRoutingProgress = (title, detail) => {
			const content = E('div', { 'class': 'oum-engine-progress' }, [
				E('div', { 'class': 'oum-act-steps' }, ['Запуск', 'Применение и проверка', 'Результат'].map(label => E('div', { 'class': 'oum-act-step', 'data-state': 'pending' }, [E('span', { 'class': 'oum-act-ico', 'aria-hidden': 'true' }), E('span', {}, label)]))),
				E('p', { 'class': 'oum-engine-progress-message', role: 'status', 'aria-live': 'polite' }, detail),
			...
			const paint = (stage, message, failed = false) => {
				content.dataset.state = failed ? 'failed' : stage === 3 ? 'success' : 'running';
				content.setAttribute('aria-busy', stage < 3 && !failed ? 'true' : 'false');
				content.querySelectorAll('.oum-act-step').forEach((step, i) => { step.dataset.state = i < stage ? 'done' : i === stage ? (failed ? 'failed' : 'active') : 'pending'; });
				content.querySelector('[role="status"]').textContent = message;
				if (stage === 3 || failed) content.querySelector('.oum-engine-progress-actions').replaceChildren(E('button', { type: 'button', 'class': 'btn cbi-button-action', click: ui.hideModal }, 'Готово'));
			};
			paint(0, detail);
			return { applying: text => paint(1, text), success: text => paint(3, text), failed: text => paint(1, text, true) };
		};
		const switchNodeConfirmed = async (target) => {
			const progress = beginRoutingProgress('Смена VPN-ноды', 'Отправляем запрос на переключение…');
			try {
				const result = await callSelectNode(target);
				if (!result.ok) throw new Error(result.message || 'Не удалось переключить ноду.');
				progress.applying('Переключаем подключение и проверяем выбранную ноду…');
				for (let attempt = 0; attempt < 90; attempt++) {
					await new Promise(resolve => window.setTimeout(resolve, 1000));
					const nodes = await callNodeStatus();
					if (nodes.applying || (result.applying && attempt < 2)) continue;
					if ((result.engine === 'passwall' ? nodes.current_id : nodes.current) !== target)
						throw new Error('Новая нода не подтверждена. Проверьте текущее подключение.');
					updateNodes(nodes);
					progress.success('Нода переключена. Настройки устройств обновлены.');
					return nodes;
				}
				throw new Error('Переключение ещё не завершено. Обновите страницу перед повторной попыткой.');
			} catch (error) { progress.failed(error.message); throw error; }
		};

		const waitDevicePolicy = async () => {
			for (let attempt = 0; attempt < 90; attempt++) {
				await new Promise(resolve => window.setTimeout(resolve, 1000));
				const fresh = await callDashboardStatus();
				if (fresh.policy_applying || attempt < 2) continue;
				if (fresh.policy_state !== 'success')
					throw new Error('Не удалось применить маршрутизацию. Повторите выбор режима.');
				updateDashboard(fresh);
				return;
			}
			throw new Error('Применение ещё не завершено. Обновите страницу перед повторной попыткой.');
		};
		...
		root.querySelector('#client-list').addEventListener('change', (ev) => {
			...
			const progress = beginRoutingProgress('Маршрутизация устройства', 'Сохраняем выбранный режим…');
			callSetDevicePolicy(target.dataset.mac, target.value).then((result) => {
				...
				progress.applying('Перестраиваем правила и проверяем результат…');
				return waitDevicePolicy().then(() => { policyMessage.textContent = 'Маршрутизация устройства применена.'; progress.success(policyMessage.textContent); });
			}).catch((err) => {
		...
		root.querySelector('#devices-panel').addEventListener('click', (ev) => {
			...
			if (action.dataset.deviceAction === 'mobile') {
				...
				const close = E('button', { type: 'button', 'class': 'btn cbi-button', 'data-device-confirm': '' }, 'Готово');
				...
				routeSelect.addEventListener('change', () => {
					message.textContent = routeSelect.value !== (client.policy || 'default') ? 'Нажмите «Готово», чтобы применить выбранный режим.' : '';
				});
				close.addEventListener('click', () => {
					if (close.disabled) return;
					if (routeSelect.value === (client.policy || 'default')) {
						document.getElementById('oum-device-custom')?.classList.remove('active');
						ui.hideModal();
						return;
					}
					...
					callSetDevicePolicy(mac, routeSelect.value).then((result) => {
						if (!result.ok) throw new Error(result.message || 'Не удалось изменить маршрутизацию.');
						progress.applying('Перестраиваем правила и проверяем результат…');
						return waitDevicePolicy().then(() => progress.success('Маршрутизация устройства применена.'));
					}).catch((error) => { progress.failed(error.message); message.textContent = error.message; routeSelect.disabled = false; close.disabled = false; });
				});
		...
		nodePanel.addEventListener('click', (ev) => {
			...
			switchNodeConfirmed(target.dataset.node).then((nodes) => {
				updateNodes(nodes);
		      ...
		      if(podkop && !document.getElementById("oum-podkop-sheet-services")){
		        ...
		        btns.forEach(b=>{
                  ...
                  pill.appendChild(document.createTextNode(" "+label));
		                  ...
		                  pill.addEventListener("click",()=>{ const vpn=row.querySelector('input[value="vpn"]'); const direct=row.querySelector('input[value="direct"]'); if(vpn&&direct){ if(vpn.checked){direct.checked=true;} else {vpn.checked=true;} row.setAttribute("aria-checked", String(vpn.checked)); pill.classList.toggle("active", vpn.checked); updateRouteSummary(); }});
          ...
          if(nodesSheet && nodesSheet.classList.contains("oum-modal") !== (window.innerWidth<=900)) {
            ...
            if(window.innerWidth<=900) {
		          ...
		          confirmBtn.addEventListener("click",()=>{
		            ...
		            refreshPickButton();
		            showNodeMessage("Переключаем ноду…",false);
		            switchNodeConfirmed(pickedNode).then((nodes)=>{
		              updateNodes(nodes);
		...
		try{
		  ...
		  L.ui.showModal = ui.showModal = function(title, content){
		    ...
		    if((isDevice || isQR) && window.innerWidth<=900){
		      ...
		      const confirm = isDevice && Array.isArray(content) ? content.map(el => el?.querySelector?.('[data-device-confirm]')).find(Boolean) : null;
		      const doneButton = sheet.querySelector('.oum-sheet-done');
		      const replacement = doneButton.cloneNode(true);
		      doneButton.replaceWith(replacement);
		      replacement.addEventListener('click', () => { if (confirm) confirm.click(); else sheet.classList.remove('active'); });
