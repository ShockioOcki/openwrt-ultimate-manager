...
return view.extend({
	...
	render(data) {
		...
		const isTunnelJob = (status) => ['podkop_configure', 'podkop_awg', 'podkop_proxy', 'podkop_youtube', 'zapret_auto', 'zapret_apply', 'zapret_check', 'zapret_restore'].includes(status.action);
		let tunnelProgress = null;
		const showTunnelJob = (status) => {
			if (!tunnelProgress) {
				const labels = status.action.startsWith('zapret_') ? ['Подготовка проверки', 'Проверка стратегии', 'Результат'] : ['Подготовка подключения', 'Активация и проверка', 'Результат'];
				const content = E('div', { 'class': 'oum-engine-progress', style: 'width:100%' }, [
					...
					E('div', { 'class': 'oum-engine-progress-bar', role: 'progressbar', 'aria-label': 'Проверка стратегий' }, [E('span', { 'class': 'oum-engine-progress-fill' })]),
					E('div', { 'class': 'oum-engine-progress-actions' })
				]);
				ui.showModal(status.action.startsWith('zapret_') ? 'Стратегия Zapret' : 'Активация туннеля', [content]);
				tunnelProgress = content;
			}
			const done = status.state === 'success', failed = status.state === 'failed';
			const stage = done ? 3 : (status.code === 'queued' ? 0 : 1);
			tunnelProgress.dataset.state = status.state;
			tunnelProgress.querySelectorAll('.oum-act-step').forEach((el, i) => { el.dataset.state = i < stage ? 'done' : i === stage ? (failed ? 'failed' : 'active') : 'pending'; });
			tunnelProgress.querySelector('.oum-engine-progress-message').textContent = status.message || 'Подготавливаем операцию…';
			const bar = tunnelProgress.querySelector('[role="progressbar"]');
			...
			if (done || failed) tunnelProgress.querySelector('.oum-engine-progress-actions').replaceChildren(E('button', { 'class': 'btn cbi-button-action', click: () => {
				ui.hideModal(); tunnelProgress = null;
		...
		const watchJob = () => {
			...
			const tick = () => callSystemJobStatus().then((status) => {
				if (isTunnelJob(status)) { statusNode.hidden = true; showTunnelJob(status); }
				...
				if (status.action === 'engine' || isTunnelJob(status)) return;
