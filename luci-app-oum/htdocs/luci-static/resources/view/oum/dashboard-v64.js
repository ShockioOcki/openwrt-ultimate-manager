...
return view.extend({
	...
	render(data) {
		...
		// An unchecked service inherits routing; only YouTube has an explicit direct mode.
		const selectedCommunities = (route) => Array.from(root.querySelectorAll('[data-community-route]:checked'))
			.filter((item) => item.value === route && (route === 'vpn' || item.dataset.communityRoute === 'youtube'))
			.map((item) => item.dataset.communityRoute).join('\n');
		...
		const updateRouteSummary = () => {
			...
			for (const row of root.querySelectorAll('[data-route-row]')) {
				...
				row.hidden = !!russiaSelected && coveredRussiaServices.has(id);
