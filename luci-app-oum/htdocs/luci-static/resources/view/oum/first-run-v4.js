...
return view.extend({
	...
	render(status) {
		if (status.setup_complete)
			return E('div', { 'class': 'oum-shell oum-first-run' }, [
			E('link', { rel: 'stylesheet', href: `${L.resource('oum/oum.css')}?v=20260916-routeicons1` }),
				...
				E('p', {}, E('a', { href: L.url('oum', 'help') + '#vpn-guide' }, 'Как настроить VPN'))
			]);
		...
		const root = E('div', {
			...
			E('div', { 'class': 'oum-setup-head' }, [
				E('div', {}, [
					...
					E('p', { 'class': 'oum-muted' }, 'Три понятных шага — и роутер готов к работе.')
				]),
				E('span', { 'class': 'oum-setup-counter', id: 'oum-setup-counter' }, 'Шаг 1 из 3')
			]),
			E('div', { 'class': 'oum-progress' }, [1,2,3].map((n) => E('span', { 'data-progress': n }))),
		...
		const update = () => {
			...
			root.querySelector('#oum-setup-counter').textContent = `Шаг ${step} из 3`;
			root.querySelector('#oum-back').disabled = step === 1;
			root.querySelector('#oum-next').textContent = step === 3 ? 'Применить настройки' : 'Продолжить';
		};
		...
		root.querySelector('#oum-next').addEventListener('click', (ev) => {
			...
			const apply = () => {
				...
				callApplySetup(
					...
					ui.showModal('Настройка завершена', [
						...
						E('p', {}, `Адрес панели остаётся прежним: ${window.location.host}. ${value('admin_password') ? 'Войдите как root с новым паролем.' : 'Войдите как root без пароля.'}`),
			...
			ui.showModal('Wi-Fi будет перезапущен', [
				E('p', {}, `После применения текущая сеть Wi-Fi будет перенастроена. Подключитесь к новой сети ${networkNames}.`),
				E('p', { 'class': 'oum-note oum-warn' }, `Запомните новое имя Wi-Fi и пароль. Адрес панели останется прежним: ${window.location.host}.`),
				E('div', { 'class': 'right' }, [
