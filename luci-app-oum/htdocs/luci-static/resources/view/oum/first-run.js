	...
	render(status) {
		...
		const root = E('div', { 'class': 'oum-shell' }, [
			...
			E('section', { 'class': 'oum-step', 'data-step': 3, 'hidden': '' }, [
				...
				cardRadio('vpn_type', 'proxy', 'Proxy', 'Одиночная ссылка VLESS Reality или другой поддерживаемый прокси.'),
				cardRadio('vpn_type', 'none', 'Пропустить', 'Добавить защищённое подключение можно позже.'),
				E('p', { 'class': 'oum-note oum-warn' }, 'В этой тестовой версии мастер запомнит выбранный тип. Импорт ключа и создание единственного профиля появятся на следующем этапе.')
			]),
