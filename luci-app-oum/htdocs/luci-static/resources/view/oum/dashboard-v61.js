	...
	render(data) {
		...
		if (!status.setup_complete) {
			const page = E('main', { 'class': 'oum-main' }, [
				E('link', { rel: 'stylesheet', href: `${L.resource('oum/oum.css')}?v=20260916-route-dark1` }),
		...
		root.querySelector('#devices-panel').addEventListener('click', (ev) => {
			...
			if (action.dataset.deviceAction === 'mobile') {
				...
				routeSelect.addEventListener('change', () => {
					...
					}).catch((error) => { progress.failed(error.message); message.textContent = error.message; routeSelect.disabled = false; });
				});
