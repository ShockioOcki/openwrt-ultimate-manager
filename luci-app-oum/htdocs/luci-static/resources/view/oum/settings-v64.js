...
const callUninstallGearup = rpc.declare({ object: 'oum', method: 'uninstallGearup', expect: { '': {} } });
...
return view.extend({
	load() {
		return Promise.all([ callSettingsStatus(), callSystemJobStatus(), callStatus(), callVpnJobStatus(), callZapretStrategyStatus(), callGearupStatus() ]);
	},
	...
	render(data) {
		...
		const gearupState = !gearup.installed ? 'не установлен' : (!gearup.bound ? 'установлен, не привязан' : (gearup.boost ? 'буст активен' : 'привязан'));
		const gearupLine = !gearup.installed ? 'Плагин не установлен.' : (!gearup.bound ? 'Установлен. Привяжите роутер в мобильном приложении GearUP.' : (`Привязан${gearup.boost ? ' · буст активен' : ''}${gearup.running ? '' : ' · процесс остановлен'}.`));
		...
		const page = E('main', { 'class': 'oum-main' }, [
			...
			...(capabilities.usb_host ? [ E('section', { 'class': 'oum-settings-panel oum-usb-panel' }, [
				...
				E('div', { 'class': 'oum-setting-actions oum-usb-actions' }, [
					...
					...(usb.runtime_ready && usb.disk ? [ E('details', { 'class': 'oum-usb-maintenance', open: window.innerWidth > 900 ? '' : null }, [E('summary', {}, 'Обслуживание накопителя'), E('button', { 'class': 'btn cbi-button-negative', id: 'format-usb-storage', 'data-system-action': '' }, 'Форматировать ext4')]) ] : [])
				]),
			...
			E('section', { 'class': 'oum-settings-panel' }, [
				E('h3', {}, 'Игровой ускоритель'),
				E('p', { 'class': 'oum-help' }, 'GearUP ускоряет игры на консолях. Буст включается в мобильном приложении GearUP; OUM ставит плагин, следит за привязкой и удаляет.'),
				E('div', { id: 'gearup-status', 'class': 'oum-wisp-status' }, gearupLine),
				E('div', { 'class': 'oum-setting-actions' }, [
					E('button', { 'class': 'btn cbi-button-action', id: 'install-gearup', 'data-system-action': '', disabled: gearup.installed ? '' : null }, 'Установить GearUP'),
					E('button', { 'class': 'btn cbi-button-negative', id: 'uninstall-gearup', 'data-system-action': '', disabled: gearup.installed ? null : '' }, 'Удалить GearUP'),
					E('button', { type: 'button', 'class': 'btn oum-mobile-sheet-cancel' }, 'Отмена')
				])
			]),
			E('section', { 'class': 'oum-settings-panel' }, [
 ...
 dns: 'Настройка DNS', adguard: 'Настройка защиты', passwall_geodata: 'Обновление GeoSite / GeoIP',
 engine_update: 'Обновление VPN-движка', project_update: 'Обновление OUM', project_rollback: 'Восстановление OUM',
 mesh_runtime: 'Подготовка Mesh', gearup_install: 'Установка GearUP', gearup_uninstall: 'Удаление GearUP',
 podkop_configure: 'Активация туннеля', podkop_awg: 'Активация AWG', podkop_proxy: 'Активация Reality', podkop_youtube: 'Маршрут YouTube',
 zapret_auto: 'Подбор стратегии Zapret', zapret_apply: 'Применение стратегии Zapret', zapret_check: 'Проверка Zapret', zapret_restore: 'Восстановление стратегии Zapret',
 zapret_quic: 'Настройка QUIC', zapret_manager_install: 'Подготовка Zapret Manager'
};
const isOperationJob = (status) => Object.prototype.hasOwnProperty.call(operationTitles, status.action);
		let operationProgress = null;
		const showOperationJob = (status) => {
			if (!operationProgress) {
				const labels = status.action === 'dns' ? ['Подготовка DNS', 'Применение и проверка DNS', 'Результат'] : status.action.startsWith('zapret_') ? ['Подготовка', 'Выполнение и проверка', 'Результат'] : ['Подготовка', 'Применение и проверка', 'Результат'];
				const content = E('div', { 'class': 'oum-engine-progress', style: 'width:100%' }, [
					E('div', { 'class': 'oum-act-steps' }, labels.map(label => E('div', { 'class': 'oum-act-step', 'data-state': 'pending' }, [E('span', { 'class': 'oum-act-ico', 'aria-hidden': 'true' }), E('span', {}, label)]))),
					E('p', { 'class': 'oum-engine-progress-message', role: 'status', 'aria-live': 'polite' }),
					E('div', { 'class': 'oum-engine-progress-bar', role: 'progressbar', 'aria-label': 'Ход операции' }, [E('span', { 'class': 'oum-engine-progress-fill' })]),
					E('div', { 'class': 'oum-engine-progress-actions' })
				]);
				ui.showModal(operationTitles[status.action], [content]);
				operationProgress = content;
			}
			const done = status.state === 'success', failed = status.state === 'failed';
			const stage = done ? 3 : (status.code === 'queued' ? 0 : 1);
			operationProgress.dataset.state = status.state;
			operationProgress.querySelectorAll('.oum-act-step').forEach((el, i) => { el.dataset.state = i < stage ? 'done' : i === stage ? (failed ? 'failed' : 'active') : 'pending'; });
			operationProgress.querySelector('.oum-engine-progress-message').textContent = status.message || 'Подготавливаем операцию…';
			const bar = operationProgress.querySelector('[role="progressbar"]');
			const total = Number(status.total), progress = Number(status.progress);
			bar.hidden = !done && !(total > 0);
			if (done || total > 0) {
				// The current strategy is still being tested; do not count it as completed.
				const percent = done ? 100 : Math.max(0, Math.min(99, Math.floor(100 * (progress - 1) / total)));
				bar.setAttribute('aria-valuenow', String(percent));
				bar.querySelector('span').style.width = percent + '%';
			}
			if (done || failed) operationProgress.querySelector('.oum-engine-progress-actions').replaceChildren(E('button', { 'class': 'btn cbi-button-action', click: () => {
				ui.hideModal(); operationProgress = null;
				acknowledgeStatus(status).finally(() => { if (done) window.location.reload(); });
			} }, 'Закрыть'));
		};
		const watchJob = () => {
			if (watching) return;
			...
			const tick = () => callSystemJobStatus().then((status) => {
				if (isOperationJob(status)) { statusNode.hidden = true; showOperationJob(status); }
				else { statusNode.hidden = false; paintStatus(status); }
				...
				if (status.action === 'engine' || isOperationJob(status)) return;
		...
		const installGearupButton = root.querySelector('#install-gearup');
		if (installGearupButton) installGearupButton.addEventListener('click', async (event) => {
			event.preventDefault();
			if (await confirmation('Установить GearUP?', 'OUM поставит TUN-модуль и плагин GearUP, настроит сохранение привязки на перезагрузку. Привязка роутера выполняется в мобильном приложении GearUP.', 'Установить', false))
				start(callInstallGearup());
		});
		const uninstallGearupButton = root.querySelector('#uninstall-gearup');
		if (uninstallGearupButton) uninstallGearupButton.addEventListener('click', async (event) => {
			event.preventDefault();
			if (await confirmation('Удалить GearUP?', 'Плагин, монитор, привязка и OUM-правила будут удалены; firewall и PassWall перезапустятся. Исключения доменов и kmod-tun останутся.', 'Удалить', true))
				start(callUninstallGearup());
		});
		...
		const usbDesktopMedia = window.matchMedia('(min-width: 901px)');
		usbDesktopMedia.addEventListener('change', event => {
			const maintenance = usbPanel?.querySelector('.oum-usb-maintenance');
			if (maintenance) maintenance.open = event.matches;
		});
		...
		const openSheet = (title, node, prepare) => {
			...
			sheet.setAttribute('aria-labelledby', 'oum-settings-sheet-title');
		...
		const openVpnSheet = (title, section) => {
				...
				try {
					...
					if (dnsSection) {
						const customDNS = E('div', { style: 'display:flex;flex-direction:column;gap:12px;' }, [
							E('div', { 'class': 'oum-sheet-card', style: 'background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:12px;' }, Array.from(dnsSection.querySelectorAll('.oum-dns-engine')).map(engine=>{
							...
							E('button', { 'class': 'btn cbi-button-action', style: 'width:100%;height:42px;background:#2563eb;color:#fff;border:none;border-radius:12px;font-weight:600;margin-top:4px;', click: () => {
								for (const id of ['dns-current', 'bootstrap-dns-current']) {
									const source = customDNS.querySelector('#' + id + '-mobile');
									const target = dnsSection.querySelector('#' + id);
									if (source && target) target.value = source.value;
								}
								document.getElementById('apply-engine-dns')?.click();
							} }, 'Применить DNS'),
							E('div', { style: 'font-size:11px;color:#6b7280;text-align:center;' }, 'VPN-движок будет кратковременно перезапущен.')
						]);
			...
			if (section === 'connection') {
				...
				if (engines.current === 'openclash' || engines.current === 'passwall') {
					...
					const cardS = E('div', { 'class': 'oum-sheet-card', style: 'background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:12px;display:flex;flex-direction:column;gap:8px;' }, [
		...
		const networkCards = Array.from(networkPanel.querySelectorAll('.oum-network-card'));
		const networkTabs = E('div', { 'class': 'oum-apple-segment', role: 'tablist', 'aria-label': 'Расширение сети', style: 'display:grid;grid-template-columns:repeat(' + networkCards.length + ',minmax(0,1fr));gap:4px;padding:3px;margin-bottom:12px;border-radius:12px;background:var(--oum-field-soft)' });
		const selectNetworkTab = index => {
			networkCards.forEach((card, i) => {
				card.hidden = i !== index;
				card.style.setProperty('display', i === index ? 'block' : 'none', 'important');
				const button = networkTabs.children[i];
				button.classList.toggle('is-active', i === index);
				button.setAttribute('aria-selected', String(i === index));
				button.style.background = i === index ? 'var(--oum-paper)' : 'transparent';
				button.style.color = i === index ? 'var(--oum-ink)' : 'var(--oum-subtle)';
			});
		};
		networkCards.forEach((card, i) => {
			card.style.cssText = 'border:0!important;background:transparent!important;box-shadow:none!important;padding:0!important;margin:0!important;min-width:0';
			networkTabs.appendChild(E('button', { type: 'button', role: 'tab', 'class': 'oum-apple-seg', style: 'border:0;border-radius:9px;padding:10px 8px;min-width:0', click: () => selectNetworkTab(i) }, i === 0 ? 'Локальная сеть' : 'Mesh-сеть'));
		});
		const networkGrid = networkPanel.querySelector('.oum-network-card-grid');
		networkGrid.style.setProperty('display', 'block', 'important');
		networkGrid.before(networkTabs);
		selectNetworkTab(0);
		...
		const mobileHub = E('section', { 'class': 'oum-mobile-settings-hub', 'aria-label': 'Разделы настроек' }, [
			...
			E('div', { 'class': 'oum-mobile-settings-launchers' }, [
				...
				...(capabilities.usb_host ? [ launcher('USB', usbSummary, () => openSheet('USB', usbPanel)) ] : []),
			...
			E('section', { 'class': 'oum-mobile-vpn-card' }, [
				...
				E('div', { 'class': 'oum-mobile-vpn-current' }, [ E('span', { 'aria-hidden': 'true' }), E('strong', {}, engineTitle), E('small', {}, engineMissing ? 'не установлен' : 'установлен') ]),
			...
			E('section', { 'class': 'oum-mobile-maintenance-hub' }, [
				E('h3', {}, 'Игровой ускоритель'),
				launcher('GearUP', `${gearupState}`, () => openSheet('Игровой ускоритель', gearupPanel))
			]),
			E('section', { 'class': 'oum-mobile-maintenance-hub' }, [
				E('h3', {}, 'Обслуживание OUM'),
				E('div', { 'class': 'oum-mobile-vpn-actions' }, maintenanceCards.map((card, index) => {
					const btn = E('button', { type: 'button' }, maintenanceTitles[index]);
					btn.addEventListener('click', () => openSheet(maintenanceTitles[index], card));
					return btn;
				}))
			])
		]);
		...
		root.classList.add('oum-settings-workspace');
