	...
	render(data) {
		...
		if (!status.setup_complete) {
			const page = E('main', { 'class': 'oum-main' }, [
				E('link', { rel: 'stylesheet', href: `${L.resource('oum/oum.css')}?v=20260903-act28` }),
		...
		const updateNodes = (fresh) => {
			...
			showAllNodes.hidden = isPasswall;
			showAllNodes.style.display = isPasswall ? 'none' : '';
			...
			const usePick = !isPasswall && window.innerWidth <= 900;
		const makeNode = (node, isCurrent, quick) => {
			...
			if (showAllNodesCount) showAllNodesCount.textContent = isPasswall ? `Показать все ${all.length}` : `Все ноды (${all.length})`;
			const sheetTitle = document.getElementById('oum-nodes-sheet-title');
			if (sheetTitle) sheetTitle.textContent = isPasswall ? `Показать все ${all.length}` : `Все ноды (${all.length})`;
