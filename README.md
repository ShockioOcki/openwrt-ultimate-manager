...
устанавливает AmneziaWG один раз вместе с OUM в неинтерактивном режиме `-e -n`.
OUM загружает скрипт из закреплённого commit и проверяет SHA-256 перед запуском;
создание интерфейса `awg1` отключено, потому что OUM управляет своим `oum_awg`.
После
первой установки пользователь один раз перезагружает роутер, чтобы модуль ядра
активировался до настройки VPN. Смена движков больше не устанавливает и не
удаляет пакеты AWG и не перезагружает роутер. При переходе с Podkop управляемый
интерфейс `oum_awg` сохраняется с `auto=0` и останавливается; при возврате он
включается с прежней конфигурацией. Чужие AWG/WireGuard-интерфейсы OUM не
затрагивает.
