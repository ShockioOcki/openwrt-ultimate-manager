#!/bin/sh
set -eu
ROOT="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)"
"$ROOT/tools/build.sh"
sh -n "$ROOT/dist/oum-test.sh"
sh -n "$ROOT/tools/build-installer.sh"
"$ROOT/tools/build-installer.sh"
"$ROOT/dist/oum-install.sh" --check
python3 "$ROOT/tests/test_release_payload.py"
python3 "$ROOT/tests/test_theme_install.py"
ruby -c "$ROOT/helpers/source_converter.rb"
ruby -c "$ROOT/luci-app-oum/root/usr/libexec/oum-policy-yaml.rb"
ruby -c "$ROOT/luci-app-oum/root/usr/libexec/oum-backup-codec.rb"
ruby -c "$ROOT/tests/test_backup_codec.rb"
sh -n "$ROOT/tools/install-luci-dev.sh"
sh -n "$ROOT/tools/install-proton.sh"
sh -n "$ROOT/tools/install-offloading.sh"
python3 "$ROOT/tests/test_install_offloading.py"
sh -n "$ROOT/tools/archive-luci-theme.sh"
sh -n "$ROOT/tools/oum-install-header.sh"
sh -n "$ROOT/tools/stage-engine-assets.sh"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-firstboot"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-source-job"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-reset-first-run"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-mihomo-api"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-openclash-nodes"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-passwall-nodes"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-passwall-policy"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-passwall-source-job"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-passwall-bypass-russia"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-passwall-geodata"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-passwall-route-check"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-device-policy"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-subscription-info"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-backup"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-reset-vpn"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-system-job"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-engine-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-podkop-config"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-awg-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-dns-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-mesh-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-mesh-runtime"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-usb-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-mobile-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-media-organizer-run"
python3 -c 'import ast, pathlib, sys; ast.parse(pathlib.Path(sys.argv[1]).read_text())' "$ROOT/luci-app-oum/root/usr/libexec/oum-media-organizer"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-project-manager"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-zapret-strategy"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-zapret-quic"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-zapret-manager"
python3 "$ROOT/tests/test_zapret_fix.py"
OUM_ZAPRET_CATALOG="$ROOT/luci-app-oum/root/usr/share/oum/zapret-youtube-strategies" "$ROOT/luci-app-oum/root/usr/libexec/oum-zapret-strategy" validate
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-login-default"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-dropbear-blank-password"
sh -n "$ROOT/tests/test_subscription_info.sh"
sh -n "$ROOT/tests/test_login_default.sh"
sh -n "$ROOT/tests/test_contracts.sh"
sh -n "$ROOT/tests/test_device_alias.sh"
sh -n "$ROOT/tests/test_device_pause.sh"
sh -n "$ROOT/tests/test_engine_ux.sh"
sh -n "$ROOT/tests/test_dashboard_compact.sh"
sh -n "$ROOT/tests/test_wisp.sh"
sh -n "$ROOT/tests/test_parental.sh"
sh -n "$ROOT/tests/test_wifi_access.sh"
sh -n "$ROOT/tests/test_health_help.sh"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-traffic"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-parental-cron"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-adguard"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-support"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-support-shell"
sh -n "$ROOT/luci-app-oum/root/usr/libexec/oum-support-repair-shell"
sh -n "$ROOT/tests/test_remote_support.sh"
sh -n "$ROOT/luci-app-oum/root/etc/uci-defaults/94_oum_parental"
sh -n "$ROOT/luci-app-oum/root/etc/uci-defaults/90_oum_firstboot"
sh -n "$ROOT/luci-app-oum/root/etc/uci-defaults/91_oum_login_default"
sh -n "$ROOT/luci-app-oum/root/etc/uci-defaults/95_oum_support"
sh -n "$ROOT/luci-app-oum/root/etc/init.d/oum-support"
grep -Fq "sprintf('%J', dispatcher.build_url('oum/setup'))" "$ROOT/luci-app-oum/root/usr/share/ucode/luci/template/themes/oum-app/header.ut"
! grep -Fq "jsonify(dispatcher.build_url('oum/setup'))" "$ROOT/luci-app-oum/root/usr/share/ucode/luci/template/themes/oum-app/header.ut"
python3 -m json.tool "$ROOT/luci-app-oum/root/usr/share/luci/menu.d/luci-app-oum.json" >/dev/null
grep -Fq "action_logout" "$ROOT/luci-app-oum/root/usr/share/ucode/luci/controller/oum.uc"
python3 -m json.tool "$ROOT/luci-app-oum/root/usr/share/rpcd/acl.d/luci-app-oum.json" >/dev/null
if command -v node >/dev/null 2>&1; then
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/menu-oum-app.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/first-run-v4.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/dashboard-v43.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/dashboard-v56.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/dashboard-v57.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/dashboard-v64.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/settings-v43.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/settings-v64.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/parental-v3.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/parental-v5.js"
	node --check "$ROOT/luci-app-oum/htdocs/luci-static/resources/view/oum/help-v5.js"
fi
printf 'syntax tests: OK\n'
